...
3. No manual Umami configuration is required. Umami has no host port and its admin UI stays private; `deploy-analytics.sh` runs an idempotent `init-umami` step inside the analytics container that creates a `dashboard-reader` Umami account (role `user`), creates one website whose allowed domains are exactly `hydroclimatex.com,www.hydroclimatex.com,lisflood.hydroclimatex.com` (owned by that reader so it can read its stats), and prints the credentials. On an existing installation it updates the matching HydroclimateX website instead of creating a duplicate, preserving its history. The script writes `UMAMI_API_USERNAME`, `UMAMI_API_PASSWORD` and `UMAMI_WEBSITE_ID` into `.env` and locks it to mode 600.
